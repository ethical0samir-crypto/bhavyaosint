...
WATERMARK = (
    ...
    f"<a href='https://t.me/LaceraOsintBot'>🤖 @LaceraOsintBot</a>"
)
...
def validate_query(q: str, mode: str) -> tuple[str, str | None]:
    ...
    if mode == "email":
        ...
        digits = re.sub(r"\D", "", q)
        if len(digits) != 12:
            return ("", "❌  <b>ɪɴᴠᴀʟɪᴅ ᴀᴀᴅʜᴀᴀʀ</b>  ›  12 digits required")
        return (digits, None)
        ...
        if a in (0, 10, 127) or (a == 172 and 16 <= b <= 31) or (a == 192 and b == 168):
            return ("", "❌  Private / local IP not allowed.")

    elif mode == "telegramid":
        tg = q.strip().lstrip("@")
        if len(tg) < 3 or len(tg) > 64:
            return ("", "❌  <b>ɪɴᴠᴀʟɪᴅ ᴛᴇʟᴇɢʀᴀᴍ ɪᴅ</b>  ›  <code>@username</code>  ʏᴀ  <code>userid</code>")
        return (tg, None)

    elif mode == "facebookid":
        fb = q.strip().lstrip("@")
        if len(fb) < 2 or len(fb) > 100:
            return ("", "❌  <b>ɪɴᴠᴀʟɪᴅ ғʙ ɪᴅ</b>  ›  <code>username</code>  ʏᴀ  <code>profile_id</code>")
        return (fb, None)

    elif mode == "instagramid":
        ig = q.strip().lstrip("@")
        if not re.match(r"^[a-zA-Z0-9_.]{1,60}$", ig):
            return ("", "❌  <b>ɪɴᴠᴀʟɪᴅ ɪɢ ɪᴅ</b>  ›  <code>@username</code>")
        return (ig, None)

    elif mode == "domain":
        dom = q.strip().lower().removeprefix("https://").removeprefix("http://").rstrip("/")
        if not re.match(r"^[a-z0-9]([a-z0-9\-\.]{1,250})[a-z0-9]$", dom) or "." not in dom:
            return ("", "❌  <b>ɪɴᴠᴀʟɪᴅ ᴅᴏᴍᴀɪɴ</b>  ›  <code>example.com</code>")
        return (dom, None)

    elif mode == "link":
        lnk = q.strip()
        if not re.match(r"^https?://", lnk) or len(lnk) < 10:
            return ("", "❌  <b>ɪɴᴠᴀʟɪᴅ ʟɪɴᴋ</b>  ›  <code>https://example.com/page</code>")
        return (lnk, None)
...
def auto_delete_with_warning(chat_id: int, msg_id: int, delay: int = AUTO_DELETE_SECS) -> None:
    ...
    try:
        warn_id = bot.send_message(
            chat_id,
            f"⏳  <i>Result will be deleted in  <b>{delay}s</b></i>",
        ).message_id
    except Exception:
        pass
    time.sleep(delay)
...
def gate(message) -> bool:
    ...
    if status == "BANNED":
        safe_reply_to(
            message,
            "🚫  <b>ᴀᴄᴄᴇss ʀᴇᴠᴏᴋᴇᴅ</b>\n<i>Your account has been suspended.</i>",
        )
        return False
    if status == "JOIN_REQ":
        safe_send_message(
...
def mk_join() -> InlineKeyboardMarkup:
    ...
    if REQUIRED_CHANNELS:
        mu.add(InlineKeyboardButton("✅  Already Joined  —  Verify", callback_data="check_join"))
    return mu
...
def cmd_start(message):
        ...
        if ref_id > 0 and ref_id != uid and add_referral(uid, ref_id):
            try:
                safe_send_message(
                    ...
                    "✦  <b>+2 credits</b> have been added to your account.",
                )
            except Exception:
                pass
    ...
    safe_send_message(
        ...
        "◈  <b>🌐 sᴏᴄɪᴀʟ & ᴡᴇʙ ᴍᴏᴅᴇs</b>\n"
        f"<i>{'┄' * 26}</i>\n"
        "  ✈️  /telegramid   <code>@username</code>\n"
        "  📘  /facebookid   <code>username / id</code>\n"
        "  📸  /instagramid  <code>@username</code>\n"
...
def cmd_help(message):
    ...
    text = (
        ...
        "◈  <b>🌐 sᴏᴄɪᴀʟ & ᴡᴇʙ ɪɴᴛᴇʟ</b>\n"
        f"<i>{DIV}</i>\n"
        "  ✈️  <b>/telegramid</b>   <code>@username</code> ʏᴀ ɪᴅ\n"
        "  📘  <b>/facebookid</b>   <code>username / page_id</code>\n"
        "  📸  <b>/instagramid</b>  <code>@username</code>\n"
        "  🌍  <b>/domain</b>       <code>example.com</code>\n"
        "  🔗  <b>/link</b>         <code>https://example.com/...</code>\n\n"
        "◈  <b>👤 ᴀᴄᴄᴏᴜɴᴛ</b>\n"
        f"<i>{DIV}</i>\n"
        ...
        "  ┄  After that: 1 credit / search\n"
        f"  ┄  Result auto-deletes in {AUTO_DELETE_SECS}s\n"
        "  ┄  ɴᴜᴍʙᴇʀ: 🇮🇳 🇵🇰 🇧🇩 ᴏɴʟʏ"
    )
    if is_own:
        text += (
...
def cmd_profile(message):
    ...
    u   = get_user(uid)
    if not u:
        return safe_reply_to(message, "❌  Failed to load profile.")
    ...
    safe_send_message(
        ...
        f"  🆓  Remaining ›  <code>{remaining} / {DAILY_LIMIT}</code>\n"
        f"  ⏱️  ʀᴇsᴇᴛ    ›  <code>{fmt_reset(daily_reset)}</code>\n\n"
        f"<i>{DIV}</i>\n"
        f"🔗  ʀᴇғᴇʀʀᴀʟs   <code>{refer_count}</code>\n"
        "<i>✦ Use /refer to earn more credits</i>",
        reply_markup=mk_buy(),
    )
...
def cmd_refer(message):
    ...
    safe_send_message(
        ...
        "🎁  ʀᴇᴡᴀʀᴅ  ›  <b>+2 ᴄʀᴇᴅɪᴛs</b> per new user\n\n"
        f"🔗  <b>Your Link</b>\n<code>{link}</code>\n\n"
        f"<i>{DIV}</i>\n"
...
def cmd_redeem(message):
    ...
    if result is None:
        safe_reply_to(message, "❌  <b>Invalid code.</b>")
    elif result == -1:
        safe_reply_to(message, "⚠️  <b>This code has already been used.</b>")
    elif result == -2:
        safe_reply_to(message, "⏳  <b>This code has expired.</b>")
    else:
        safe_reply_to(
            message,
...
@bot.message_handler(commands=["number", "email", "vehicle", "aadhar", "pan", "ip",
                                "telegramid", "facebookid", "instagramid", "domain", "link"])
def cmd_lookup(message):
    ...
    MODE_EMOJI = {
        "number":      "📞", "email":      "📧", "aadhar":     "🪪",
        "pan":         "💳", "vehicle":    "🚗", "ip":         "🌐",
        "telegramid":  "✈️", "facebookid": "📘", "instagramid":"📸",
        "domain":      "🌍", "link":       "🔗",
    }
    MODE_PROMPT = {
        "telegramid":  (
            f"✈️  <b>Enter Telegram ID</b>\n"
            f"<i>{DIV}</i>\n"
            "  📌  <code>@username</code>  ʏᴀ  <code>123456789</code>"
        ),
        "facebookid":  (
            f"📘  <b>Enter Facebook ID</b>\n"
            f"<i>{DIV}</i>\n"
            "  📌  <code>username</code>  ʏᴀ  <code>page_id</code>"
        ),
        "instagramid": (
            f"📸  <b>Enter Instagram ID</b>\n"
            f"<i>{DIV}</i>\n"
            "  📌  <code>@username</code>"
        ),
        "domain":      (
    ...
    if len(args) < 2:
        ...
        if cmd == "number":
            prompt = (
                f"{emoji}  <b>Enter Phone Number</b>\n"
                f"<i>{DIV}</i>\n"
                "🇮🇳  <code>+91XXXXXXXXXX</code>  ʏᴀ  <code>eg-9914173314</code>\n"
                "🇵🇰  <code>+92XXXXXXXXXX</code>\n"
                "🇧🇩  <code>+880XXXXXXXXXX</code>"
            )
        elif cmd in MODE_PROMPT:
            prompt = MODE_PROMPT[cmd]
        else:
            prompt = f"{emoji}  <b>Enter {cmd.upper()}</b>"
...
def do_search(message, mode: str, query: str = None) -> None:
    ...
    if query is None:
        try:
            ...
            msg_age = 0
        if msg_age > 300:
            safe_reply_to(message, "⏱️  <b>Session timed out.</b>  Please send the command again.")
            return
    ...
    if raw_q.startswith("/"):
        ...
        return
    if not raw_q:
        safe_reply_to(message, "❌  Please enter something.")
        return
    ...
    if mode == "number":
        ...
        if not e164:
            safe_reply_to(
                ...
                "  🇮🇳  <code>+91XXXXXXXXXX</code>  (after country code, 10 digits)\n"
                "  🇮🇳  <code>XXXXXXXXXX</code>  (bare 10 ᴅɪɢɪᴛ = ɪɴᴅɪᴀ)\n"
                "  🇵🇰  <code>+92XXXXXXXXXX</code>\n"
                "  🇧🇩  <code>+880XXXXXXXXXX</code>\n\n"
                "<i>⚠️ No spaces — write number continuously</i>",
            )
            return
        q = e164
    ...
    user = get_user(uid)
    if not user:
        safe_reply_to(message, "❌  Failed to load user data.")
        return
    ...
    if daily_used >= DAILY_LIMIT and credits <= 0 and uid not in OWNER_IDS:
        safe_reply_to(
            ...
            "Buy credits or refer friends to continue.",
            reply_markup=mk_buy(),
        )
        return
    ...
    if is_query_locked(q):
        def _stealth_deny(cid):
            ...
            safe_send_message(cid, "🔍  <b>No records found.</b>")
    ...
    try:
        ...
        if not results or not isinstance(results, list):
            safe_send_message(message.chat.id, "🔍  <b>No records found.</b>")
            return
        ...
        if is_real and uid not in OWNER_IDS:
            if not deduct_credit_atomic(uid, DAILY_LIMIT):
                safe_reply_to(message, "⚠️  <b>Daily Limit Reached</b>", reply_markup=mk_buy())
                return
        if is_real:
...
def cmd_stats(message):
    ...
    text = (
        ...
        f"<i>{DIV}</i>\n"
        f"🔍  Today's Searches ›  <code>{today}</code>\n"
...
def cmd_makecode(message):
    ...
    if amt <= 0 or amt > 10000:
        return safe_reply_to(message, "❌  Amount must be between 1–10000.")
    ...
    if create_code(code, amt, expiry):
        ...
        safe_reply_to(message, "❌  Failed to create code.")
...
def cmd_usedcode(message):
    ...
    if not info:
        return safe_reply_to(message, "❌  Code not found.")
    used_status = "✅ Already Used" if info["used"] else "🟢 ᴀᴠᴀɪʟᴀʙʟᴇ"
    user_part = "—"
    if info["used_by"]:
...
def cmd_giveall(message):
    ...
    mu = InlineKeyboardMarkup()
    mu.add(
        InlineKeyboardButton(f"✅  +{amt} to All", callback_data=f"giveall_confirm_{amt}"),
        InlineKeyboardButton("❌  ᴄᴀɴᴄᴇʟ", callback_data="admin_cancel"),
    )
    safe_reply_to(
        message,
        f"⚠️  <b>ᴄᴏɴғɪʀᴍ ɢɪᴠᴇᴀʟʟ</b>\n<i>{DIV}</i>\n"
        f"Give <code>+{amt} credits</code> to all users?",
        reply_markup=mu,
    )
...
def cmd_addcredits(message):
    ...
    if new_bal is not False and new_bal is not None:
        ...
        safe_reply_to(message, "❌  Error or user not found.")
...
def cmd_userlist(message):
    ...
    users = get_all_users_detail()
    if not users:
        return safe_reply_to(message, "📭  No users found.")
    total  = len(users)
    banned = sum(1 for u in users if u[4] == 1)
...
def cmd_userinfo(message):
    ...
    u = get_user(target)
    if not u:
        return safe_reply_to(message, "❌  User not found.")
...
def cmd_detail(message):
    ...
    if not logs:
        return safe_reply_to(message, f"📭  No search history found for  <code>{target}</code>.")
...
def cmd_ban(message):
    ...
    mu = InlineKeyboardMarkup()
    mu.add(
        InlineKeyboardButton("🚫  Yes, Ban User", callback_data=f"ban_confirm_{target}"),
        InlineKeyboardButton("❌  ᴄᴀɴᴄᴇʟ", callback_data="admin_cancel"),
    )
    safe_reply_to(
        message,
        f"⚠️  Ban user <code>{target}</code>?",
        reply_markup=mu,
    )
...
def cmd_unban(message):
    ...
    safe_reply_to(message, f"✅  <code>{parts[1]}</code>  has been unbanned.")
...
def cmd_unlock(message):
    ...
    if remove_lock(q):
        ...
        safe_reply_to(message, "⚠️  Query not found in lock list.")
...
def cmd_listlocked(message):
    ...
    if not locked:
        return safe_reply_to(message, "📭  No locked queries found.")
...
def handle_cb(call):
    ...
    if call.data == "check_join":
        ...
        if is_joined(uid):
            ...
            safe_answer_callback(call.id, "❌  Please join all required channels first!", show_alert=True)
    ...
    # ── New search ──
        ...
        safe_send_message(
            ...
            "📞 /number  📧 /email  🪪 /aadhar\n"
            "💳 /pan  🚗 /vehicle  🌐 /ip\n\n"
            "✈️ /telegramid  📘 /facebookid  📸 /instagramid\n"
            "🌍 /domain  🔗 /link",
        )
    ...
    # ── Pagination ──
        ...
        if entry is None:
            return safe_answer_callback(call.id, "⚠️  Session expired. Please search again.", show_alert=True)
        try:
            p = max(0, int(p_str))
        except ValueError:
            return safe_answer_callback(call.id)
    ...
    # ── Copy plain text ──
        ...
        with _cash_lock:
            if qid not in cash_reports:
                return safe_answer_callback(call.id, "⚠️  Session expired.", show_alert=True)
    ...
    # ── Ban confirm ──
        ...
        ban_user(target)
        safe_edit_message(
            f"🚫  <code>{target}</code>  has been banned.",
            call.message.chat.id, call.message.message_id,
        )
    ...
    # ── Giveall confirm ──
        ...
        if give_all_credits(amt):
            safe_edit_message(
                f"🎁  <b>Done!</b>  All users received <code>+{amt} credits</code>.",
                call.message.chat.id, call.message.message_id,
            )
        safe_answer_callback(call.id)
